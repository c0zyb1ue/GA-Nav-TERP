...
class Env():
    def __init__(self, is_training):
        ...
        self.husky_vel_topic_name = '/jackal_velocity_controller/cmd_vel'
        # self.grid_map_topic_name = '/grid_map_visualization/elevation_grid'
        self.grid_map_topic_name = '/projected_map'
        # self.scan_topic_name = '/camera/depth/color/points'
    ...
    def getElevationMap(self, data):
        ...
        print(f"height: {map_height}, width: {map_width}")
    ...
    # def convert_pointcloud_to_ranges(pointcloud_msg, max_range=8):
    #     scan_ranges = []
    ...
    #     for p in points:
    #         x, y, z = p
    #         distance = np.sqrt(x**2 + y**2)  # XY 평면상의 거리 계산
    #         if distance > max_range:  # 최대 거리 제한
    #             scan_ranges.append(max_range)
    #         else:
    ...
    #     max_range = 8  # 최대 감지 범위 (m)
    #     min_range = 0.6  # 최소 감지 범위 (m)
    #     max_robot_pose_angle = 30  # 최대 허용 기울기
    #     done = False
    #     arrive = False
    #     velocity = self.vel_cmd

    #     # 오리엔테이션 정보 (기울기 체크)
    #     pitch_deg = abs(round(np.degrees(self.pitch)))
    #     roll_deg = abs(round(np.degrees(self.roll)))

    #     # ✅ 장애물 데이터 분석을 위해 PointCloud2 데이터를 읽음
    ...
    #         for p in points:
    #             x, y, z = p
    #             distance = np.hypot(x, y)  # XY 평면상의 거리 계산

    #             # 장애물 감지 조건
    #             if min_range <= distance <= max_range and z < 1.5:  # 1.5m 이하 장애물만 고려
    #                 obstacles_detected = True
    #                 break

    #     except Exception as e:
    #         rospy.logerr(f"PointCloud2 데이터 처리 오류: {e}")
    #         return None

    #     # ✅ 이동 제한 조건 체크
    #     if velocity.linear.x < 0 or velocity.linear.x > 1:
    #         if abs(velocity.angular.z) > 0.3:
    #             rospy.logwarn("----- Velocity limits exceeded !!  -----")
    #             done = True

    #     if abs(self.position.z) > 12:
    #         rospy.logwarn("----- Spawn location error !!  -----")
    #         done = True
    ...
    #         rospy.logwarn("------- Unsafe orientation !!  --------")
    #         done = True

    #     # ✅ 목표 도착 여부 체크
    #     current_distance = np.hypot(self.goal_position.position.x - self.position.x,
    ...
    #     return self.heading_gradient, current_distance, self.rel_theta, pitch_deg, roll_deg, self.diff_angle, done, arrive
    ...
    def step(self, action, grid_goal,waypoint_goal,grid_arrived,waypoint_arrived):
        ...
        while data is None:
            try:
                ...
                # data = rospy.wait_for_message(self.scan_topic_name, PointCloud2, timeout=5)
    ...
    def reset(self,goal_in):
        ...
        print(f"goalX_rob: {goalX_rob}")
        print(f"goalY_rob: {goalY_rob}")
    ...
    def arr_normalize(self,x):
        ...
        x = (x - xmin)/(xmax - xmin)
        # print(f"xmax: {xmax}, xmin: {xmin}, x: {x}")
    ...
    # My code!!!!!!!!!!
    def waypoint_planner(self,cbam_out, elevation_map,without_attention, grids):
    # def waypoint_planner(self,cbam_out, elevation_map,without_attention):
        ...
        self.costmap_normalized[16,18] += grids[0]
        self.costmap_normalized[16,19] += grids[1]
        self.costmap_normalized[17,18] += grids[2]
        self.costmap_normalized[17,19] += grids[3]
        
        print("&*&*&*&*&*&*&*&*&*&*&*&")
        #cv2.imshow('asdf', self.costmap_normalized)
